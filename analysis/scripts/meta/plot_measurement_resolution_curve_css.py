...
def return_data_css(df):
	...
	sz1 = (N3-N4)/(N3+N4)
	sz2 = (N6-N5)/(N5+N6)
	return (sz1-sz2), photons, N

def return_data_css_guess(df):
	#get neta data
	N3 = np.array(df['cavity_scan_analysis', 'Neta_guess_3'])
	N4 = np.array(df['cavity_scan_analysis', 'Neta_guess_4'])
	N5 = np.array(df['cavity_scan_analysis', 'Neta_guess_5'])
...
if __name__ == '__main__':
	try:
		df = data()
		# print(df)
		y, x, N = return_data_css(df)
		photons_measured = x[-1]
		bins = np.logspace(1.3,4,12)
		...
		if analyze_guessed_Neta :
			try:
				yg, xg, Ng = return_data_css_guess(df)
				photons_measured = x[-1]
		...
		xph = np.logspace(np.log10(20),np.log10(20000))
		mph = 200
		alpha = 3000
		yph = (mph/xph+xph/mph)/alpha
		plt.plot(
			xph,
			yph,
			label=f"$(\\bar{{N}}/N+N/\\bar{{N}})/\\alpha$\n$\\alpha={alpha}$\n$\\bar{{N}}={mph}$")
		plt.legend()
