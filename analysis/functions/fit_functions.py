...
def fit_rabi_splitting_transmission_MLE(data, bnds={"fatom_range":(0,50), "fcavity_range":(0,50), "Neta_range":(0,20000)}, param_error = 'off', bs_repetition = 25, path=None, bin_interval=0.2, DEBUG = False):
	'''
	...
	if Neta_guess > 0 :	Neta_gridpoints = np.array([Neta_guess])
	if fix_fatom:
...
def fit_rabi_splitting_transmission_Clustering(data, bnds={"fatom_range":(0,50), "fcavity_range":(0,50), "Neta_range":(0,20000)}, param_error = 'off', bs_repetition = 25, path=None, bin_interval=0.2, DEBUG = False):
	'''
	Extract the best fit parameters by clustering the photon's frequency data. It will identify if there are 2 peaks, and get
	output = fit_rabi_splitting_transmission_MLE(data,bnds,param_error,bs_repetition)
	...
	[] Get higher order corrections in fcav and fatoms
	[] IMplement bootstrapping method
	'''
	try:
		run = Run(path)
		data_globals = run.get_globals() # path should be called inside the function.
		if DEBUG: print("Globals imported successfully during clustering Fit") 
	except:
		print("Failed Importing Globals during clustering fit ")
	...
	best_param= (fatoms_guess, fcavity_guess, Neta_guess, gamma_loc, kappa_loc, dark_counts)
