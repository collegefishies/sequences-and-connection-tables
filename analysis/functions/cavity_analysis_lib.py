...
def fit_using_maximum_likelihood_estimator(photon_frequencies, bin_width, hdf_path, DEBUG=False):
...
def fit_splitting_using_least_squares(photon_frequencies, cavity_range, bin_width,hdf_path, DEBUG=False):
	'''
...
def fit_splitting_using_maximum_likelihood_estimator(photon_frequencies, bin_width,cavity_range, hdf_path, DEBUG=False):
	try:
...
def fit_splitting_using_clustering(photon_frequencies, bin_width,cavity_range, hdf_path, DEBUG=False):
	try:
		best_param = fit_functions.fit_rabi_splitting_transmission_Clustering(
			data=photon_frequencies, 
			bnds={"fatom_range":(0,50), "fcavity_range":cavity_range, "Neta_range":(0,200000)},
			bin_interval=bin_width,
			param_error = 'off',
			path=hdf_path
		)
...
def fit_using_gaussian_mixture_model(photon_frequencies, bin_width = None,cavity_range = None,  hdf_path=None, DEBUG=False,plt=None):
...
def fit_using_Clustering(photon_frequencies, bin_width = None,cavity_range = None,  hdf_path=None, DEBUG=False,plt=None):
	'''
	Vestigial code, can be removed (also from atoms_in_cavity_helper.py)
