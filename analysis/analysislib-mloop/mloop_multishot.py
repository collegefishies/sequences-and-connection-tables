...
def update_globals(new_globals):
    msg = """Updating globals with desired run value."""
    rm.set_globals(new_globals)
    return msg
...
def verify_globals(config):
    ...
    if not np.array_equal(current_values, requested_values):
        ...
        message += update_globals(requested_dict)
...
def cost_analysis(cost_key=(None,), maximize=True, x=None):
    ...
    N = int(list(df['number_of_shots'])[-1])
    if N != len(df):
        print("Not enough shots")
        print("Waiting for more shots")
        #return if not all shots are identical
        return {'bad': True}


    # Retrieve cost from specified column
    if len(df) and cost_key in df:
        #use the last N shots
        cost = np.nanmean((df[cost_key].astype(float).values))
        u_cost = np.nanstd((df[cost_key].astype(float).values))
...
if __name__ == '__main__':
    # config = mloop_config.get(r"C:\Users\YbMinutes\labscript-suite\userlib\labscriptlib\ybclock\scripts\mloop\mloop_config.ini")
